...
fn main() -> Result<()> {
    ...
    let config: Config = toml::from_str(toml_str).context("assets/dev.toml must parse into Config")?;
...
async fn seed_mood_entries(
    ...
    seed_entry(pool, config, "content", "", h(17) + 1800, &[("mood_notes", "2")]).await?;
    ...
    seed_entry(pool, config, "reflective", "", h(8), &[("notes", "morning thoughts")]).await?;
    seed_entry(pool, config, "reflective", "", h(23), &[("notes", "evening reflection")]).await?;

    // --- yesterday ---------------------------------------------------------
    seed_entry(pool, config, "productive", "", yh(9), &[("steps", "3000")]).await?; // lower bin
    seed_entry(pool, config, "neutral", "", yh(12), &[("temperature", "20")]).await?; // in range
    ...
    // UTC-day apart.
    seed_entry(pool, config, "tired", "", yh(12) + 30 * 60, &[("sleep", "6"), ("water", "2")]).await?;

    // --- two days ago ------------------------------------------------------
    seed_entry(pool, config, "hot", "", two_days_ago + 12 * 3600, &[("temperature", "35")]).await?; // above max
    ...
    seed_entry(pool, config, "tired", "", last_week + 14 * 3600, &[("run_times", "90")]).await?;
    ...
    const MONTH_MOODS: &[&str] = &["happy", "great", "neutral", "tired", "energized", "content", "hydrated"];
    for i in 2..=29 {
        if i % 10 == 0 {
    ...
    const YEAR_MOODS: &[&str] = &["happy", "sad", "drained", "charged", "passive", "purposeful"];
...
async fn seed_oneshot_tasks(
    ...
    // No date (start = now).
    seed_task(pool, "buy groceries", "", 5, Some(now), None, None, 0, false, None).await?;
    // Today at 9:00.
    seed_task(pool, "meeting", "", 5, Some(today + 9 * 3600), None, None, 0, false, None).await?;
    // Recent past (yesterday) — shows up in @due.
    seed_task(pool, "dentist appointment", "", 5, Some(yesterday + 10 * 3600), None, None, 0, false, None).await?;
    // Old past with a body (specific datetime).
    let old = date::parse_datetime("2024-03-20 14:30:00", date::DateDialect::Uk)?;
    seed_task(pool, "team sync", "sync notes", 5, Some(old), None, None, 0, false, None).await?;
    // Completed oneshot (completion entry of 1).
    let id = seed_task(pool, "finish report", "", 5, Some(yesterday + 15 * 3600), None, None, 1, false, None).await?;
    update_task(pool, id, 1).await?;
    // Partial multi-count oneshot: target 3 with one completion (● 1/3 badge).
    let id = seed_task(pool, "launch checklist", "", 5, Some(today + 8 * 3600), None, None, 3, false, None).await?;
    update_task(pool, id, 1).await?;
    // Priority extremes (color binning).
    seed_task(pool, "urgent deploy", "", 9, Some(today + 10 * 3600), None, None, 0, false, None).await?;
    seed_task(pool, "someday read", "", 1, Some(tomorrow + 9 * 3600), None, None, 0, false, None).await?;
    // Optional task.
    seed_task(pool, "stretch", "", 5, Some(today + 12 * 3600), None, None, 0, true, None).await?;
    // Future (tomorrow) — not yet due.
    seed_task(pool, "plan trip", "", 5, Some(tomorrow + 9 * 3600), None, None, 0, false, None).await?;
...
async fn seed_recurring_tasks(pool: &sqlx::SqlitePool, now: i64) -> Result<()> {
    ...
    let id = seed_task(pool, "recurring_1", "", 5, Some(now), None, Some(86400), 0, false, None).await?;
    update_task(pool, id, 3).await?;
    // Weekly with a 2-day availability window (recurring + availability).
    seed_task(pool, "recurring_2", "weekly review", 7, Some(now), Some(2 * 86400), Some(7 * 86400), 0, false, None).await?;
    // Daily, stops after 3 days (end_time).
    seed_task(pool, "recurring_3", "", 5, Some(now), None, Some(86400), 0, false, Some(now + 3 * 86400)).await?;
    // Daily with a target of 5, optional.
    seed_task(pool, "recurring_4", "", 5, Some(now), None, Some(86400), 5, true, None).await?;
    // Daily with a body.
    seed_task(pool, "recurring_5", "standup notes", 5, Some(now), None, Some(86400), 0, false, None).await?;
...
async fn seed_scheduled_tasks(
    ...
    seed_task(pool, "meditate", "", 10, Some(now - 30 * 60), Some(2 * 3600), None, 0, false, None).await?;
    // Future today: window overlaps today (start < today_end ∧ start+dur > today_start).
    seed_task(pool, "call mom", "", 10, Some(today + 18 * 3600), Some(3600), None, 0, false, None).await?;
    // Elapsed, no entry → auto-completed (shows in @done without include_completed).
    seed_task(pool, "clean desk", "", 10, Some(now - 3 * 3600), Some(3600), None, 0, false, None).await?;
    // Completed early: explicit entry of 1.
    let id = seed_task(pool, "write blog", "", 10, Some(now - 30 * 60), Some(2 * 3600), None, 0, false, None).await?;
    set_scheduled_completion(pool, id, 1).await?;
    // Failed: explicit entry of 0.
    let id = seed_task(pool, "pay bills", "", 10, Some(now - 30 * 60), Some(2 * 3600), None, 0, false, None).await?;
    set_scheduled_completion(pool, id, 0).await?;
    // Fully past (yesterday) — listed in @due (start < today_end).
    seed_task(pool, "water plants", "", 10, Some(yesterday + 10 * 3600), Some(3600), None, 0, false, None).await?;
...
fn custom_value(config: &Config, name: &str, raw: &str) -> Result<CustomValue> {
    ...
    Ok(match tracker.kind {
        ...
        TrackerType::Number => CustomValue::Number(
            raw.parse()
                .with_context(|| format!("Cannot parse '{raw}' as an integer for tracker '{name}'"))?,
        ),
        TrackerType::Float => CustomValue::Float(
            raw.parse()
                .with_context(|| format!("Cannot parse '{raw}' as a number for tracker '{name}'"))?,
        ),
    })
}
