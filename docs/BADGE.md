...
| State | Glyph | Color |
|---|---|---|
| any mood entry | `●` | Oklab mood color (embedded label → color, cached) |
...
| State | Glyph | Color |
|---|---|---|
| journal-only entry (empty mood label) | configured `journal_badge`, or none | `DarkGray` |
...
| --- | --- | --- |
| numeric / float | `◆` | `bin_score_color` (binned by score) |
| text | `◆` | `DarkGray` |
