...
use crate::today::EntryKind;
use crate::types::TaskKind;
use crate::types::ViewVariant;
...
use super::{build_preview, confirm_buttons, suspend::edit_with_editor, truncate_chars, Render};
...
pub struct TodayApp {
    ...
    pub(crate) entries: Vec<crate::today::TodayEntry>,
    pub(crate) selected: usize,
    pub(crate) horizon: crate::types::TodayHorizon,
...
/// enum remains today-app-specific because it also owns tracker editing.
...
impl TodayApp {
    pub async fn new(
        ...
        day_epoch: Option<i64>,
        show: ViewVariant,
        horizon: crate::types::TodayHorizon,
    ) -> Self {
        let mut color_cache = std::collections::HashMap::new();
        let entries = crate::today::fetch_today_entries(
            pool,
            &config,
            horizon,
            day_epoch,
    ...
    async fn refresh(&mut self) {
        self.entries = crate::today::fetch_today_entries(
            &self.pool,
            &self.config,
            self.horizon,
            self.day_epoch,
    ...
    fn apply_sort(&mut self) {
        if self.sort_by_priority {
            ...
            // (crate::today::today_sort): timed first by timestamp, then the
    ...
    async fn fetch_selected_task(&mut self) {
        ...
        self.selected_task = match &entry.recurring_window {
            ...
            _ => crate::db::fetch_task_by_id(&self.pool, task_id, crate::date::now())
                .await
                .ok()
                .flatten(),
        };
    ...
    /// modals.
    async fn run_enter_action(&mut self, task: crate::db::TaskRow) {
    ...
    async fn reset_selected_progress(&mut self) {
        ...
        let task = match self.selected_task.as_ref() {
            ...
            _ => match crate::db::fetch_task_by_id(&self.pool, id, crate::date::now())
                .await
                .ok()
                .flatten()
            {
            ...
            self.modal.as_mut()
        {
            ...
            if proceed {
                let task = match self.selected_task.as_ref() {
                    ...
                    _ => {
                        match crate::db::fetch_task_by_id(&self.pool, task_id, crate::date::now())
                            .await
                            .ok()
                            .flatten()
                        {
    ...
    async fn update_tracker_score(&self, id: i64, kind: crate::config::TrackerKind, value: &str) {
        let _ = crate::db::update_tracker_score(&self.pool, id, kind, value).await;
    }
}
                ...
                if let Some(entry) = self.entries.get(self.selected) {
                    match entry.kind {
                        ...
                        EntryKind::Tracker(_) => {
                            if let Some(id) = entry.id {
                                if let Err(e) =
                                    crate::db::delete_tracker_entry(&self.pool, id).await
                                {
                        ...
                        EntryKind::Task(_) => {
                            if let Some(task_id) = entry.task_id {
                                if let Err(e) = crate::db::delete_task(&self.pool, task_id).await {
...
fn render_today_entry_list(f: &mut Frame, app: &TodayApp, area: Rect) {
    ...
    let horizon_suffix = if app.horizon == crate::types::TodayHorizon::Today {
        String::new()
    } else {
