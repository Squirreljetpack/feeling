...
impl ColorAxes {
    ...
    pub async fn build_async(
        ...
        let v_base =
            crate::embedding::get_or_embed_cached(pool, embedder, &settings.base_string, "")
                .await?;

        let mut basis_moods = Vec::with_capacity(pairs.len());
        for pair in pairs {
            let s = crate::embedding::get_or_embed_cached(
                pool,
                embedder,
                &pair.mood,
            ...
            let norm_vector = crate::embedding::normalize(&diff);
        ...
        for i in 0..n {
            for j in 0..n {
                gram_matrix[i][j] =
                    crate::embedding::dot(&basis_moods[i].vector, &basis_moods[j].vector);
            }
        }
